...
def test_remote_start_with_mock(monkeypatch, admin_user):
    ...
    resp = client.post(reverse("remote_start", args=["EVSE-100"]),
                       {"id_tag": "RFID-100"}, format="json")
...
def test_remote_stop_with_mock(monkeypatch, admin_user):
    ...
    resp = client.post(reverse("remote_stop", args=["EVSE-101"]),
                       {"transaction_id": 1}, format="json")
    assert resp.status_code == 200
    assert resp.json()["response"]["status"] == "Accepted"
